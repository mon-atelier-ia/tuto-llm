...
cells.append(
    code(
        ...
        "        if pos < 0:\n"
        '            char_id = char_to_id["."]  # avant le debut -> padding (.)\n'
        "        else:\n"
        "            char_id = char_to_id[mot[pos]]\n"
        "        vecteur.extend(embeddings[char_id])  # ajouter les 8 nombres\n"
        "    return vecteur\n"
        "\n"
        "\n"
        "# Exemple : pour predire la 5e lettre de 'pikachu'\n"
        'mot = ".pikachu."\n'
        "position = 4  # on veut predire 'a' (position 4)\n"
...
cells.append(
    code(
        ...
        "\n"
        "# Les poids du reseau (initialises au hasard)\n"
        "W = [[random.gauss(0, 0.3) for _ in range(vocab_size)] for _ in range(INPUT_DIM)]\n"
        "b = [0.0] * vocab_size  # biais (un par lettre)\n"
        "\n"
...
cells.append(
    code(
        ...
        "            scores[j] += contexte[i] * W[i][j]  # multiplication + accumulation\n"
        "    return scores\n"
        "\n"
        "\n"
        "def softmax(scores):\n"
        '    """Transforme les scores en probabilites (entre 0 et 1, somme = 1)."""\n'
        "    max_s = max(scores)  # stabilite numerique\n"
        "    exps = [math.exp(s - max_s) for s in scores]  # exponentielle\n"
        "    total = sum(exps)  # normalisation\n"
        "    return [e / total for e in exps]"
    )
)

# ----------------------------------------------------------------
# CELL 14: Test predictions + barres (code)
# ----------------------------------------------------------------
cells.append(
    code(
...
cells.append(
    code(
        ...
        "            nb += 1\n"
        "\n"
        "            # Gradient simplifie : ajuster W et b\n"
